...
## 2026-09-12 — 装配层支持按目标 DPI 自动缩小贴纸

- `forge.py assemble` 新增 `--target-dpi`：当资产按默认格位排版后低于目标分辨率时，只缩小实体尺寸并保持居中，不对位图插值放大；不传参数时保持原有排版行为。
- 目标不得低于 300 dpi；缩小后任一方向低于 20 mm 硬下限仍会拒绝，不能用不可用的小尺寸换取虚假达标。
- 交付报告与 `manifest.json` 记录目标 DPI 和自动缩小数量，CLI 同步打印；新增 6 项回归测试，单测总数 35 → 41。
- 资产或排版校验全部通过后才创建输出目录并复制原图，失败单不残留原图副本或半套交付。
- 同步更新 `AGENTS.md`、`CONTEXT.md`、架构、资产契约、印刷规格、ChatGPT Plus 流程与接手指南：适合 ChatGPT 贴纸功能导出的约 512 px 真透明素材；正式排版尺寸会变小，不能把它表述成图片变清晰或像素增加。
